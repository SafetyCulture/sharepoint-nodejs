  ...
  return {
    ...
    uploadFile: function uploadFile(list, fileLocation) {
      ...
      return readFile(fileLocation).then(function (data) {
        ...
        return api._axios.post(listURI(list) + '/' + folder + '/Files/Add(url=\'' + fileName + '\', overwrite=true)?$expand=ListItemAllFields', data, { headers: headers }).then(function (response) {
    ...
    attachFileToItem: function attachFileToItem(list, itemId, fileLocation) {
      ...
      return readFile(fileLocation).then(function (data) {
        var headers = { 'content-length': data.length };
        return api._axios.post(listURI(list) + '/items({' + itemId + ')/Files/Add(FileName=\'' + fileName + '\', overwrite=true)', data, { headers: headers });
      });
    }
  };
}