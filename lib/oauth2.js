...
// We could not make this work, consider removing.
...
function OAuth2() {
  var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { authorizeUri: AUTHORIZE_URL, tokenUri: TOKEN_URL };
  ...
  return {
    ...
    getClientId: function getClientId() {
      var id = this.realm ? this.clientId + '@' + this.realm : this.clientId;
      _logger.log.info('Add-in full client ID = ' + id + ' Realm = ' + this.realm + ' Short client ID = ' + this.clientId);
      return id;
    },
    post: function post(params) {
      _logger.log.info('POST to ' + tokenUri + ' with ' + _querystring2.default.stringify((0, _lodash.omit)(params, 'refresh_token')));
      ...
      return (0, _requestPromise2.default)({
        ...
        try {
          ...
          _logger.log.error('Failed to deserialise POST response: ' + response);
          throw err;
        }
      }, function (err) {
        _logger.log.error('POST failed with ' + err);
      });
    }
  };
}