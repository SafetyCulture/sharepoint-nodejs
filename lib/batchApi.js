...
exports.SharePointBatchAPI = exports.LIST_TYPES = undefined;
...
var listURI = function listURI(title) {
...
var SharePointBatchAPI = exports.SharePointBatchAPI = function () {
  function SharePointBatchAPI(host, auth) {
    _classCallCheck(this, SharePointBatchAPI);

    if (!host) throw new Error('SharePointBatchAPI requires host string');
    if (!auth) throw new Error('SharePointBatchAPI requires auth object');
  ...
  */
  ...
  _createClass(SharePointBatchAPI, [{
    key: 'run',
    value: function run() {
      var _this = this;
      ...
      return _axios2.default.post(this.host + '/_api/$batch', data, {
        ...
        return Promise.all(res.data.split('--batchresponse').map(function (batchRes) {
          ...
          return responses.filter(_lodash2.default.identity);
        });
      });
    }
    ...
    */
    ...
    value: function addChangeset(changes) {
      ...
      var changesBody = _lodash2.default.map(changes, function (change) {
    ...
    */
    ...
    value: function _create(resource, body) {
      return ['POST ' + this.host + '/_api/web' + resource + ' HTTP/1.1', 'Content-Type: application/json;odata=verbose', '', JSON.stringify(fillSpaces(body)), ''].join('\r\n');
    }
  }, {
    key: '_delete',
    value: function _delete(resource, body) {
      return ['POST ' + this.host + '/_api/web' + resource + ' HTTP/1.1', 'Content-Type: application/json;odata=verbose', 'IF-MATCH: etag or "*"', 'X-HTTP-Method: DELETE', '', JSON.stringify(fillSpaces(body)), ''].join('\r\n');
    }
    ...
    */
    ...
    value: function _update(resource, body) {
      return ['PATCH ' + this.host + '/_api/web' + resource + ' HTTP/1.1', 'Content-Type: application/json;odata=verbose', 'Accept: application/json;odata=verbose', 'If-Match: "1"', '', JSON.stringify(fillSpaces(body)), ''].join('\r\n');
    }
    ...
    */
    ...
    value: function createList(title, description, fields) {
      ...
      var baseTemplate = arguments.length <= 3 || arguments[3] === undefined ? LIST_TYPES.STANDARD : arguments[3];
      ...
      this.addChangeset(_lodash2.default.map(fields, function (fieldType, field) {
        return _this2._change(_this2._create(listURI(title) + '/fields', {
